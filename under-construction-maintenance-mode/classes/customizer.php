...
class UCMM_WPBrigade_Entities {
	...
	public function customize_ucmm_wpbrigade( $wp_customize ) {
		...
		/**
		 * Sanitize the input for the analytics script.
		 *
		 ...
		 * @since 1.5.2
		 */
		function ucmm_sanitize_analytics_script( $input ) {
			// Define the allowed HTML tags and attributes for the Google Analytics script.
			$allowed_html = array(
				'script' => array(
					'async'       => array(),
					'src'         => array(),
			...
			// Use wp_kses to sanitize the input, allowing the specified tags.
			$sanitized_script = wp_kses( $input, $allowed_html );

			// Additional sanitization: ensure the src attribute points to a valid Google domain.
			if ( strpos( $sanitized_script, 'https://www.googletagmanager.com/gtag/js?id=' ) === false ) {
				// If the src attribute does not contain the expected Google Analytics URL, reject the script.
				return '';
			}
	...
	endforeach;
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'capability'        => 'manage_options',
				'transport'         => 'postMessage',
				'sanitize_callback' => 'ucmm_sanitize_analytics_script',
			)
		);
