...
if ( ! is_admin() || ! current_user_can( 'manage_options' ) ) {
	wp_die(
		esc_html__( 'You do not have permission to access this page.', 'ucmm-wpbrigade' ),
		'',
		array( 'response' => 403 )
	);
}
...
if ( ! defined( 'WPBRIGADE_SDK__DEV_MODE' ) || true !== WPBRIGADE_SDK__DEV_MODE ) {
	wp_die(
		esc_html__( 'Debug mode is not enabled.', 'ucmm-wpbrigade' ),
		'',
		array( 'response' => 403 )
	);
}
...
$wpb_debug_msg_success = __( 'Successfully set the option.', 'ucmm-wpbrigade' );
$wpb_debug_msg_error   = __( 'Option not set. Name must start with wpb_.', 'ucmm-wpbrigade' );
?>
...
<p class="notice notice-warning" style="margin: 1em 0;" role="alert">
	<strong><?php esc_html_e( 'Admin-only debug page.', 'ucmm-wpbrigade' ); ?></strong>
	<?php esc_html_e( 'This page shows sensitive data (keys, paths, user info). Do not share screenshots or leave unattended.', 'ucmm-wpbrigade' ); ?>
</p>

<h2><?php esc_html_e( 'Actions', 'ucmm-wpbrigade' ); ?></h2>
<table>
	<tbody>
		<tr>
			<td>
			...
			<td>
				...
				<button type="button" class="button" id="set_option_button"><?php esc_html_e( 'Set DB Option', 'ucmm-wpbrigade' ); ?></button>
				<form id="set_option_form" method="post" style="display: none; margin-right: 10px;">
					...
					<div class="option-input-wrapper" style="display: inline-block;">
						<label for="option_name"><?php esc_html_e( 'Option Name (must start with wpb_):', 'ucmm-wpbrigade' ); ?></label>
